      ...
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-4xl font-bold font-serif flex items-center gap-3"><Map className="h-9 w-9 text-primary" />Tourism Directory</h1>
              <p className="text-muted-foreground mt-1">Discover the landmarks, culture, and attractions of Constituency No. 32</p>
            </div>
            <div className="flex rounded-lg border border-border overflow-hidden">
              <button onClick={() => setLang("en")} className={`px-4 py-2 text-sm font-semibold font-display transition-all ${lang === "en" ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}>English</button>
              <button onClick={() => setLang("st")} className={`px-4 py-2 text-sm font-semibold font-display transition-all ${lang === "st" ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}>Sesotho</button>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-wrap gap-3 mb-6">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search places…" className="w-full pl-10 pr-4 py-2 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
          </div>
          <div className="flex gap-2 flex-wrap">
            {["All", ...cats].map(c => (
              <button key={c} onClick={() => setCatFilter(c)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold font-display border transition-all ${catFilter === c ? "bg-primary text-primary-foreground border-primary" : "border-border hover:border-primary/40"}`}>
                {CAT_ICONS[c] && <span className="mr-1">{CAT_ICONS[c]}</span>}{c}
              </button>
            ))}
          </div>
        </motion.div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20"><Map className="h-14 w-14 text-muted mx-auto mb-3" /><p className="text-muted-foreground">No places found</p></div>
        ) : (
              ...
              <motion.div key={p.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="heritage-card p-5 flex flex-col">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold font-display leading-snug">{lang === "en" ? p.name : (p.nameST || p.name)}</h3>
                    <span className="badge-gold text-xs">{p.category}</span>
                  </div>
                </div>
                <p className="text-sm text-muted-foreground flex-1 mb-3 line-clamp-4">
                  {lang === "en" ? p.description : (p.descriptionST || p.description)}
                </p>
